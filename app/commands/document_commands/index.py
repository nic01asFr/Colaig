...
async def faiss_index_command(ep: EventParser, matrix_client: MatrixClient):
    ...
    try:
        ...
        if not session_context:
            ...
            session_context = await context_manager.create_context(
                ...
                ContextType.SESSION,
                {
                    ...
                    "history": [],
                    "conversation_state": {
                        "command_type": "index",  # Préfixe unique pour identifier le type de commande
                        "last_command": "index",
            ...
            # Mettre à jour le contexte de session avec l'état modifié
        ...
        # Important : ne pas utiliser le gestionnaire de contexte async with pour éviter la fermeture prématurée des connexions
        ...
        try:
            ...
            # Toujours appeler close() pour nettoyer proprement les ressources, mais pas avant que tout soit terminé
            if hasattr(index_service, 'close'):
                await index_service.close()
            
    except Exception as e:
        logger.error(f"Erreur lors du traitement de la commande !index: {str(e)}")
