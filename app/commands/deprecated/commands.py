...
from app.services.context.instance import get_context_manager  # Import de la fonction getter au lieu du singleton
...
async def albert_reset(ep: EventParser, matrix_client: MatrixClient):
    ...
    if config.albert_with_history:
        ...
        await get_context_manager().delete_context(context_id)
        
        # Création d'un nouveau contexte
        await get_context_manager().create_context(
            ep.room.room_id,
            ep.sender,
            config=config.dict()
        )
...
async def process_request(ep: EventParser, matrix_client: MatrixClient):
    ...
    try:
        ...
        try:
            room_context = await get_context_manager().get_or_create_room_context(
            ...
            await get_context_manager().add_room_participant(ep.room.room_id, ep.sender)
            await get_context_manager().update_room_activity(ep.room.room_id, ep.sender)
        ...
        try:
            session_context = await get_context_manager().get_context(session_id, ContextType.SESSION)
            
            if not session_context:
                # Créer le contexte de session
                session_context = await get_context_manager().create_context(
                    session_id,
                    ContextType.SESSION,
                    {
        ...
        try:
            request_context = await get_context_manager().create_context(
        ...
        try:
            ...
            await get_context_manager().update_context(
                session_id,
                ContextType.SESSION,
