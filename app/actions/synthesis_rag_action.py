...
class SynthesisRagAction:
    ...
    async def retrieve_comprehensive_chunks(
        ...
        """
        Récupère un ensemble complet de chunks pertinents pour la synthèse
        
        Args:
            query: La requête ou sujet de la synthèse
            session_context: Contexte de la session utilisateur (optionnel)
        ...
        if not self.index_service:
            logger.info("Initialisation des services pour la synthèse...")
        ...
        try:
            # Récupérer l'ID du salon à partir des contextes
            room_id = None
            if session_context:
                room_id = session_context.room_id
            if room_context:
