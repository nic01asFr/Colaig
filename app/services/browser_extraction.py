...
async def extract_web_content_optimized(url: str, config) -> Dict[str, Any]:
    ...
    try:
        ...
        agent = Agent(
            ...
            # Plan par défaut pour guider l'agent même avec planification limitée
            default_plan=[
                "Accéder à l'URL spécifiée",
                "Attendre le chargement complet de la page",
                "Identifier et extraire le titre principal",
                "Identifier et extraire le contenu principal",
...
async def extract_web_content(url: str, config) -> Dict[str, Any]:
    ...
    try:
        ...
        if _browser_use_available:
            ...
            # Plan par défaut pour pallier au manque de planification du modèle
            default_plan = [
                "Ouvrir le navigateur et accéder à l'URL",
                "Attendre le chargement complet de la page (5 secondes)",
            ...
            agent = Agent(
                task=extraction_task,
                llm=AlbertAgentWrapper(api_key=api_key, base_url=base_url, model=model),
                default_plan=default_plan  # Ajouter un plan par défaut si le modèle ne peut pas planifier
            )
...
async def extract_with_httpx(url: str) -> Dict[str, Any]:
    """
    Méthode d'extraction de contenu de secours utilisant httpx.
    Utilisée lorsque browser-use échoue.
    
    Args:
        url: URL de la page à extraire
        
    Returns:
        Dict: Contenu de la page avec métadonnées de base
    """
    try:
        logger.info(f"Tentative d'extraction de secours avec httpx pour {url}")
        ...
        # Créer un client HTTP avec redirection automatique et timeouts adaptés
        async with httpx.AsyncClient(headers=headers, follow_redirects=True, timeout=30.0) as client:
            ...
            # Tenter d'extraire le titre (balise title)
            ...
            # Pour une extraction plus sophistiquée, il faudrait utiliser BeautifulSoup
            # Mais pour une solution de secours simple, on nettoie le HTML avec des regex
            ...
            # Recherche de contenu principal pour Légifrance
            if "legifrance.gouv.fr" in url:
                # Recherche de patterns spécifiques à Légifrance
                jorf_match = re.search(r'JORFTEXT\d+', url)
                if jorf_match:
                    jorf_id = jorf_match.group(0)
                    logger.info(f"Identifié texte Légifrance: {jorf_id}")
                    
                    # Recherche des sections importantes dans le contenu
                    corps_texte_match = re.search(r'corpsTexte[^>]*>(.*?)<', html_content, re.DOTALL)
                    if corps_texte_match:
                        corps_texte = corps_texte_match.group(1)
                        logger.info(f"Trouvé section corpsTexte dans Légifrance")
                        content = corps_texte + "\n\n" + content
            
            # Tronquer le contenu si trop long pour le débogage
            if len(content) > 1000:
                logger.info(f"Extrait de contenu httpx (1000 premiers caractères): {content[:1000]}...")
            else:
                logger.info(f"Contenu extrait avec httpx: {content}")
        ...
        logger.error(f"Erreur lors de l'extraction avec httpx: {str(e)}")
        return {
            "title": url,
            "content": f"Impossible d'extraire le contenu. Erreur: {str(e)}",
            "url": url,
            "status": 0,
            "extraction_method": "httpx-error"
        } 