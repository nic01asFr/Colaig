...
class SessionContext(BaseContext):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'SessionContext':
        """Crée une instance depuis un dictionnaire avec gestion des paramètres obligatoires.
        
        Cette méthode surcharge celle de la classe parente pour gérer les cas
        où les paramètres requis manquent.
        """
        data_copy = data.copy()
        ...
        # Retirer les champs qui ne sont plus utilisés (compatibilité avec versions précédentes)
        data_copy.pop('name', None)
        data_copy.pop('is_direct', None)
        data_copy.pop('participants', None)  # Champ de RoomContext
        data_copy.pop('shared_context', None)  # Champ de RoomContext
        
        # S'assurer que les paramètres obligatoires sont présents
        room_id = data_copy.get('room_id', 'unknown_room')
        
        # Si user_id n'est pas présent, essayer de l'extraire du nom du fichier ou utiliser une valeur par défaut
        user_id = data_copy.get('user_id')
        if not user_id:
            # Essayer d'utiliser le dernier élément du nom du contexte comme user_id
            context_type = data_copy.get('_type', '')
            if isinstance(context_type, str) and '@' in context_type:
                user_id = context_type.split('@')[-1]
            else:
                user_id = 'unknown_user'
            # Ajouter le user_id au dictionnaire pour l'initialisation
            data_copy['user_id'] = user_id
            
        # Si session_id n'est pas présent, le reconstruire à partir de room_id et user_id
        if 'session_id' not in data_copy:
            data_copy['session_id'] = f"{room_id}_{user_id}"
            
        # Créer l'instance
        try:
            instance = cls(**data_copy)
        except TypeError as e:
            logger.warning(f"Erreur lors de la création de SessionContext depuis un dictionnaire: {str(e)}")
            # En cas d'échec, construire avec les paramètres minimaux
            session_id = data_copy.get('session_id', f"{room_id}_{user_id}")
            instance = cls(
                session_id=session_id,
                room_id=room_id,
                user_id=user_id
            )
            # Copier les données non-essentielles si disponibles
            if 'history' in data_copy:
                instance.history = data_copy['history']
            if 'conversation_state' in data_copy:
                instance.conversation_state = data_copy['conversation_state']
...
class RoomContext(BaseContext):
    ...
    webdav_context: Optional[str] = field(default=None)  # Chemin de l'espace documentaire associé
    ...
    def set_documentation_space(self, space_path: str) -> None:
        """Définit l'espace documentaire associé à ce salon"""
        self.webdav_context = space_path
        self.update_shared_context("webdav_context", space_path)
        self.last_activity = get_synchronized_time()
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        data.update({
            ...
            "webdav_context": self.webdav_context
        })
        return data
